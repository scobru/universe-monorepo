...
library UniswapV2Helper {
  ...
  function getAmountsOut(
    ...
    for (uint256 i; i < path.length - 1; ) {
      ...
      amounts[i + 1] = getAmountOut(amounts[i], reserveIn, reserveOut, feeFactor);
      unchecked {
        i++;
      }
    }
  ...
  function getAmountsOutFromPairs(
    ...
    require(tokensPath.length == pairsPath.length + 1, "UniswapV2Library: INVALID_PATH");

    amounts = new uint256[](tokensPath.length);
    amounts[0] = amountIn;
    for (uint256 i; i < pairsPath.length; ) {
      (uint256 reserveIn, uint256 reserveOut) = getReserves(pairsPath[i], tokensPath[i], tokensPath[i + 1]);
      amounts[i + 1] = getAmountOut(amounts[i], reserveIn, reserveOut, feeFactors[i]);
      unchecked {
        i++;
      }
    }
  ...
  function calcSimpleZap(
    ...
    tokenAmountOut = getAmountOut(swapAmount, reserveInput, reserveOutput, feeFactor);
  }
  ...
  function swap(
    ...
    (uint256 amountOutA, uint256 amountOutB) = tokenIn == token0 ? (uint256(0), amountOut) : (amountOut, uint256(0));
  ...
  function swapExactTokensForTokens(
    ...
    uint256[] memory amountsOut = getAmountsOutFromPairs(amountIn, feeFactors, pairsPath, tokensPath);
    ...
    for (uint256 i = 0; i < pairsPath.length; ) {
      (address token0, ) = sortTokens(tokensPath[i], tokensPath[i + 1]);
  ...
  function swapExactTokensToLiquidity1(
    ...
    tokenAmountOut -= 1;
    zapLiquidityConfig.lpSubtokenIn.safeTransfer(tokenOut, amountOut - swapAmount);
