...
import {
  BeefyMultiFarm__factory,
  BeefyMultiFarmFactory__factory,
...
async function main() {
  ...
  const BeefyMultiFarmFactory = new BeefyMultiFarmFactory__factory(signers[0])
  const beefyMultiFarmFactory = await upgrades.deployProxy(
    BeefyMultiFarmFactory,
    [AGGREAGATOR, externalProxyCall.address, WETH, TREASURY],
    {
      initializer: 'initialize'
    }
  )
  console.log('BeefyMultiFarmFactory Deployed: ' + beefyMultiFarmFactory.address)
  const beefyMultiFarm = await new BeefyMultiFarm__factory(signers[0]).deploy()
